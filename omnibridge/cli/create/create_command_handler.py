...
def add_hugging_face(args: Dict[str, Any]) -> None:
    name = args['name']
    api_key: ApiKey = JsonDataManager.load(["api keys", args['key']], ApiKey)
    wrapper: HuggingFaceWrapper = HuggingFaceWrapper(name=name, api_key=api_key.value, model=args['sub_model'])
    JsonDataManager.save(["models", name], wrapper)
